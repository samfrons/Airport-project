...
function Skeleton({ className = '', style }: { className?: string; style?: React.CSSProperties }) {
  return (
    <div
      className={`animate-pulse bg-zinc-800/60 ${className}`}
      style={style}
    />
  );
}
...
export function MapSkeleton() {
  return (
    <div className="bg-zinc-900 border border-zinc-800 h-[480px] lg:h-[580px] flex items-center justify-center">
...
export function TableSkeleton({ rows = 8 }: { rows?: number }) {
  return (
    <div className="bg-zinc-900 border border-zinc-800">
      ...
      {/* Header */}
      <div className="flex px-5 py-3 border-b border-zinc-800 gap-6">
      ...
      {Array.from({ length: rows }).map((_, i) => (
        <div
          key={i}
          className="flex items-center px-5 py-3 border-b border-zinc-800/50 gap-6"
        >
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800">
      ...
      <div className="p-5 space-y-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="border border-zinc-800/60 px-3 py-2.5 space-y-2">
        ...
        <div className="bg-zinc-900 border border-red-900/30 p-6">
          <div className="text-center space-y-2">
            ...
            <button
              ...
              className="mt-2 px-4 py-1.5 text-[10px] font-medium bg-zinc-800 text-zinc-400 hover:text-zinc-200 border border-zinc-700 transition-colors"
            >
              Try Again
            </button>
          </div>
